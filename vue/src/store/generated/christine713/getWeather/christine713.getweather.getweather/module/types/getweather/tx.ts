...
const baseMsgQueryWether: object = { creator: "", title: "", body: "" };

export const MsgQueryWether = {
  encode(message: MsgQueryWether, writer: Writer = Writer.create()): Writer {
    if (message.creator !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgQueryWether {
    ...
    const message = { ...baseMsgQueryWether } as MsgQueryWether;
...
const baseMsgQueryWetherResponse: object = {};

export const MsgQueryWetherResponse = {
  encode(_: MsgQueryWetherResponse, writer: Writer = Writer.create()): Writer {
    return writer;
  },
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgQueryWetherResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseMsgQueryWetherResponse } as MsgQueryWetherResponse;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        default:
...
export class MsgClientImpl implements Msg {
  ...
  QueryWether(request: MsgQueryWether): Promise<MsgQueryWetherResponse> {
    ...
    const promise = this.rpc.request(
      "christine713.getweather.getweather.Msg",
      "QueryWether",
      data
    );
    return promise.then((data) =>
      MsgQueryWetherResponse.decode(new Reader(data))
    );
  }
}
